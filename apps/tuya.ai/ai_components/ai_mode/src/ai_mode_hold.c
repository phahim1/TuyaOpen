/**
 * @file ai_mode_hold.c
 * @brief ai_mode_hold module is used to 
 * @version 0.1
...
do { \
    PR_DEBUG("mode hold state change form %s to %s", ai_get_mode_state_str(_old),\
...
static OPERATE_RET __ai_mode_hold_init(void)
{
...
static OPERATE_RET __ai_mode_hold_task(void *args)
{
...
static OPERATE_RET __ai_mode_hold_handle_event(AI_NOTIFY_EVENT_T *event)
{
...
static AI_MODE_STATE_E __ai_mode_hold_get_state(void)
{
...
static OPERATE_RET __ai_mode_hold_handle_key(TDL_BUTTON_TOUCH_EVENT_E event, void *arg)
{
    ...
    switch(event) {
        ...
        case TDL_BUTTON_PRESS_UP: {
            if(sg_is_wakeup) {
                MODE_STATE_CHANGE(sg_mode_set_state, AI_MODE_STATE_UPLOAD);
            }
        }
        break;
        default:
...
OPERATE_RET ai_mode_hold_register(void)
{
    ...
    handle.name         = HOLD_TALK;
    handle.init         = __ai_mode_hold_init;
    ...
    TUYA_CALL_ERR_RETURN(ai_mode_register(AI_CHAT_MODE_HOLD, &handle));
