/**
 * @file ai_mode_oneshot.c
 * @brief ai_mode_oneshot module is used to 
 * @version 0.1
...
do { \
    PR_DEBUG("mode oneshot state change form %s to %s", ai_get_mode_state_str(_old),\
...
static OPERATE_RET __ai_mode_oneshot_init(void)
{
...
static OPERATE_RET __ai_mode_oneshot_deinit(void)
{
    tuya_ai_input_stop();
...
static OPERATE_RET __ai_mode_oneshot_task(void *args)
{
...
static OPERATE_RET __ai_mode_oneshot_handle_event(AI_NOTIFY_EVENT_T *event)
{
    TUYA_CHECK_NULL_RETURN(event, OPRT_INVALID_PARM);

    if(event->type != AI_USER_EVT_MIC_DATA && event->type != AI_USER_EVT_TTS_DATA) {
        PR_DEBUG("[====ai_oneshot] event type: %d", event->type);
    }
...
static AI_MODE_STATE_E __ai_mode_oneshot_get_state(void)
{
    return sg_mode_set_state;
}

static OPERATE_RET __ai_mode_oneshot_client_run(void *data)
{
...
static OPERATE_RET __ai_mode_oneshot_vad_change(AI_AUDIO_VAD_STATE_E vad_flag)
{
    if(false == sg_is_wakeup) {
        return OPRT_OK;
    }

    PR_DEBUG("[====ai_oneshot] vad: [%d]", vad_flag); 
...
static OPERATE_RET __ai_mode_oneshot_handle_key(TDL_BUTTON_TOUCH_EVENT_E event, void *arg)
{
    OPERATE_RET rt = OPRT_OK;
...
OPERATE_RET ai_mode_oneshot_register(void)
{
    ...
    handle.client_run   = __ai_mode_oneshot_client_run;
    handle.vad_change   = __ai_mode_oneshot_vad_change; 
